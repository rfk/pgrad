...
each agent by explicitly talking about what it has observed, independant
...
The observation-based approach will also allow us to work direcly
with the local information available to each agent without needing
to make statements about knowledge. For example, when planning the
cooperative execution of a task, we can formulate a reactive plan
...
outcomes corresponding to each action in $s$:\begin{gather*}
...
specify it using a single axiom as described in Section \ref{sec:Background:SC:Axioms}.
