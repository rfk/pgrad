...
how agents can reason about the world using their observations, and
Section \ref{sec:Observations:Discussion} concludes with some general
discussion.
...
observations, agents in our formalism can conclude that certain actions
...
a shared task, when we must explicitly reason about both the global
...
actions of \citet{degiacomo98execution_monitoring} but significantly
more flexible.
...
this is a query about a specific situation term, it can be answered
...
could potentially have that history:\[
\Dt\,\models\,\forall s:\, History(s)=h\,\rightarrow\,\phi[s]\]


While this query universally quantifies over situation terms, we know
from the definition of $History$ precisely how many actions have
been performed, so we can pose the query in a form suitable for regression:\[
\Dt\,\models\,\forall c_{1},\dots,c_{n}:\, History(do([c_{1},\dots,c_{n}],S_{0}))=h\,\rightarrow\phi[do([c_{1},\dots,c_{n}],S_{0})]\]


Agents can use this query to reason hypothetically about the state
of the world after some sequence of actions, for example for the purposes
of planning. They could not, however, answer queries about the current
state of the world in this manner, since they can only be expected
to have their current local view $v$. They would need to pose a query
like the following:\[
\Dt\,\models\,\forall s:\, View(agt,s)=v\,\rightarrow\,\phi[s]\]


This is a much more difficult query in general. Since we do not know
how many actions have occurred based on the view $v$, we cannot re-write
this into a form suitable regression. The situation calculus currently
...
how many actions have occurred. It can construct a query like the
following to reason about the current state of the world using standard
...
It also means we can reason about observations without requiring explicit
epistemic reasoning, an ability we will utilise in Chapter \ref{ch:jointexec}.
...
domain dynamics. In particular, we can explicitly represent and reason
about asynchronous domains.
