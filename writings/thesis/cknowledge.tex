...
contain various test conditiosn and variable re-binding \emph{inside}
...
asynchronous domanis where some inductive reasoning is required. \\


As disucssed in Section \ref{sec:CKnowledge:Reasoning}, reasoning
...
language would seem to require simlar use of free variables to control
...
possible by $agt$ after the occurrenece of $a$ are a \emph{superset}
of the situations considered possible by $agt$ before the occurrence
of $a$:\[
K_{0}(agt,s',s)\,\rightarrow\, K_{0}(agt,s',do(a,s))\]


Thus actions that are always hidden from an agent, such as asyncrhonous
