...
reified action terms a complete axiomatisation of each agent's epistemic
...
The next step is to adopt the language of dynamic logic to express
...
Apart from the $\pi^{*}$ case, which we will discuss separately,
...
it should not be used as an implementation technique in practise,
...
and the PDL tableaux rules described in \citep{abate07twb_pdl}, there
is substantial work still be done towards this goal.\\


There is also an important open question left by our work so far:
does FODL \emph{precisely} characterise the expressivity required
to regress common knowledge? This chapter has provided an upper bound
on the required expressivity by formulating a sound and complete regression
rule for FODL knowledge modalities. It is also clear that the standard
$\Knows$ and $\CKnows$ modalities do not have the required expressivity.
...
Our epistemic path regressor constructs paths with a very regular
...
also applies to our group-level formalism. This would, however, be
a promising avenue for future research -- can an approach similar
to the one developed in this chapter provide effective reasoning techniques
for other group-level mental attitudes in the situation calculus,
such a mutual belief or joint goals? \\
