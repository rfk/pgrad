...
\begin{defnL}
[{Context~Free~Sucessor-State~Axioms}] A successor state
...
predicates that take an \noun{Action} term are are modified to take
...
Predicates and functions taking actions are modified to take \noun{Concurrent\#Timepoint
}pairs, e.g. $do(c,s)$ becomes $do(c\#t,s)$. The new function $start$
...
This next step is then performed immediately, and the process repats
...
The event calculus is further removed, in that it contains a single
