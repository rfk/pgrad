...
may be found at the beginning of search subsequent chaper.
...
programming system, which forms the basis of our implementation.
...
version \citep{levesque98sc_foundations}.
...
description predicates, and the corresponding $<_{\alpha}$ notation.


\subsection{Notation and Axioms\label{sec:Background:SC:Notation-and-Axioms}}
...
agent indicated by their first argument; \noun{Situation} terms are
histories of the actions that have occurred in the world, with the
initial situation represented by $S_{0}$ and successive situations
built using the function $do\,:\, Action\times Situation\rightarrow Situation$;
...
These primitive fluents can be generalised to \emph{uniform formulae},
which represent more complex properties of the state of the world.
They are basically logical combinations of fluents referring to a
common situation term. For the moment we restrict ourselves to \emph{objective}
uniform formulae; the complete definition includes statements about
knowledge and will be introduced following that material.
...
with the situation argument removed from all its fluents. For example,
if $\phi$ represents $Holding(Ann,Knife,s)\wedge Holding(Bob,Bowl,s)$,
...
primitive fluents change between situations ($\Dt_{ssa}$); axioms
describing the value of primitive fluents in the initial situation
($\Dt_{S_{0}}$); and axioms giving situation-independent background
facts ($\Dt_{bg}$):\[
\Dt=\Sigma\cup\Dt_{una}\cup\Dt_{ad}\cup\Dt_{ssa}\cup\Dt_{S_{0}}\cup\Dt_{bg}\]
...
arguments are in fact different. It also defines the function $actor(a)$
to give the agent performing an action, which is always the action's
first argument.
...
pickup an object if nobody is current holding it:\[
...
for each type of action $A$. In principle any number of predicates
and functions can be defined in this way, and we will use the meta-variable
$\alpha$ to represent an arbitrary action description predicate.
...
account of the world state. There is a distinguished initial situation
$S_{0}$ called the \emph{initial situation}. Situations in general
form a tree structure with the initial situation at the root and $do(a,s)$
constructing the successor situation resulting when the action $a$
is performed in $s$. We abbreviate the performance of several successive
actions by writing:\[
...
only first-order logic (FOL):\[
\Dt\models_{SOL}\exists s:\,\phi(s)\,\,\,\,\mathrm{iff}\,\,\,\,\Dt\models_{FOL}\exists s:\,\phi(s)\]
...
\Reg(Poss(a,s))\isdef & \,\,\, a=A_{1}(\vars{x}_{1})\wedge\Reg(\Phi_{Poss}^{A_{1}}(\vars{x}_{1},s))\,\vee\,\dots\,\vee\, a=A_{n}(\vars{x}_{n})\wedge\Reg(\Phi_{Poss}^{A_{n}}(\vars{x}_{n},s))\\
...
from section \ref{sec:Background:SC:Notation-and-Axioms}.
