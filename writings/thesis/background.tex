...
\begin{defnL}
[{{[}{{[}{Basic~Action~Theory}]}]}] A basic action theory,
denoted $\Dt$, is a set of situation calculus sentences (of the specific
...
\begin{defnL}
[{{[}{{[}{Objective~Uniform~Formulae}]}]}] Let $s$ be a fixed
situation term, $F$ an arbitrary fluent, $\vars{x}$ a vector of
terms mentioning no situation term other than $s$, and $y$ a variable
that is not of sort \noun{Situation}. Then the objective formulae
uniform in $s$ are the smallest set of syntactically-valid formulae
satisfying:\[
...
\begin{defnL}
[{{[}{{[}{Uniform~Formulae}]}]}] Let $s$ be a fixed situation
term, $F$ a primitive fluent, $\alpha$ an action description predicate,
