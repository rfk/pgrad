...
One popular option is to assume that the \noun{Action} and \noun{Object}
...
In preparation for the developments later in this chapter, we break
slightly with standard situation calculus notation and introduce an
additional fluent $K_{0}(agt,s',s)$ to model the initial epistemic
uncertaintly of the agents, with the corresponding macro $\KnowsZ(agt,\phi,s)$
...
To model initial epistemic uncertaintly, the foundational axioms $\Sigma$
...
extensions of the foundational axioms presented in Section TODO:\begin{gather*}
Init(s)\,\rightarrow\,\neg(s'\sqsubset s)\\
...
The definition of legal situations is modified to permit alternate
roots:\[
Legal(s)\,\isdef\, root(s)\leq_{Legal}s\]


Finally, we restrict the initial epistemic uncertaintly of the agents
to be about initial situations only, so they initially know that no
...
Explicitly define Exogenous actions
