...
A fundamential aspect of epistemic reasoning is identifiying certain
...
\item [{{Euclidian:}}] $K(agt,s_{2},s_{1})\wedge K(agt,s_{3},s_{1})\rightarrow K(agt,s_{3},s_{2})$ 
...
ocurrance of an action should not, for example, remove the agent's
...
by directly modifying the successor state axiom for $K$ {[}TODO:
refs here]. The axiom typically gorws from the form presented in equation
\eqref{eq:k_ssa_standard} to look something like the following:\begin{gather*}
...
if and only if it is satisified at $s$. Unfortunately, these extensions
...
In order to account for the actions of other agents being completly
...
this chapter, by basing our approach on an explicit, separately-axiomatied
...
the guard formula itself also becomes known. While this can be achived
...
Consider how guarded sensing actions can be axiomatied using explicit
obervations, as discussed in Section \ref{sec:Observations:Guarded-Sensing}
...
macros, regressing it at $S_{0}$ and then supressing the situation
...
\Reg(\mathbf{Knows}(agt,\phi,o\cdot v))\isdef\,\, & \mathbf{Knows}(agt,\forall c:\, Obs(agt,c)=o\\
 & \,\,\,\,\,\,\,\,\wedge Legal(c)\rightarrow\Reg(\Pst(\phi,\LbU(agt)),c),v)\\
\Reg(\mathbf{Knows}(agt,\phi,\epsilon))\isdef\,\, & \KnowsZ(agt,\Pst(\phi,\LbU(agt)),S_{0})\end{align*}
...
own knowledge using only their local information, even in asyncrhonous
...
syncrhonous domains. It is straightforward to show that $\Pst(\phi,\LbU(agt))$
in syncrhonous domains is always equivalent to $\phi$. The regression
...
domains where some inductive reasonig is required.
...
\ref{ch:intro}. We adopt an explicit axiomatisation of partial obervability
...
repectively. By formulating ordinary successor state axioms for these
...
}an explicit successor state axiom for $\Knows_{A}(agt,F,s)$ and
...
the \emph{past }of the possible situation $s''$. Based on our explicitly
formalisation of the agent's local view, our framework introduces
hidden actions in the \emph{future }of $s''$, avoiding this unintuitive
behavior.
...
in asynchronous domains -- if defintion \ref{defn:Observations:View}
...
the agent is required to consider any possible sequence of hidden
actions.
...
can acquire significant amounts of knowledge.

One possible approact to alleviating this requirement is to assume
that agents always know the current time, or some bound on the current
time. They can use this information in conjunction with their local
view to determine what they know, given that the current time is $\tau$:\[
\Dt\cup\Dt_{K}^{obs}\,\models\,\Knows(agt,(start<\tau)\rightarrow\phi,v)\]


Since this query is uniform, it can be approched directly using our
regresion rules. If all actions take some finite duration, $\tau$
...
of the persistene condition calculation.

Another option would be to abandon the requiement that the agents
...
using a possible-worlds approach simlar to that used for knowledge
...
We do not consider belief-based systems in this thesis, but now that
the use of a similar possible-worlds semantics means that the notions
of observations and views could also provide benefits for such formalisms.\\
...
queries, including a preliminary implementation of such a reasoning
system.
