...
as its semantics have a straightforward encoding as a logic program.
...
in the case of multiple individual control programs as well. It is
presented in Algorithm \ref{alg:readylog_exec}.
...
coordinate their actions without the need for explicit communicatione.
...
support for predicting natural actions. This means we must specify
...
agree in advance on a convention for grounding these variants, for
example by setting them to their smallest possible value. To simplfy
...
a planned execution. Of course, the standard IndiGolog approach is
still required if there are unpredictable exogenous actions that must
...
then have a straightforward encoding as Oz procedures, using the \textbf{case}
...
SharedPlans \citep{grosz99sharedplans}, and TAEMS \citep{decker95designing_coordination}.
...
is that limitations in the reasoning and planning machinery of the
situation calculus tend to restrict the multi-agent domains to which
it can be applied. This first incarnation of MIndiGolog has inherited
these limitations.
