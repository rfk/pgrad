...
observable domain, and discuss the challenges faced when moving to
an online execution model.
...
be exponentially-many histories in general. In Section \ref{sec:JointExec:Reasonable}
we show how to avoid the need to reason about each history individually,
which is crucial if these structures are to be of practical use.
...
be legal in the real world. A joint execution may have action events
enabling outcome events that they would never produced under the given
theory of action. It may call for actions to be performed in situations
wheere they are not legal, or allow actions to be performed concurrently
...
agent's ability to carry out the plan in practise.
...
semantics of MIndiGolog from Chapter \ref{ch:mindigolog}.
...
things are not so straightforward. While it is straightforward to
extend a joint execution one action at a time, it is difficult to
coordinate such a procedure between agents.

Recall the ReadyLog execution algorithm from Algorithm \ref{alg:readylog_exec},
which our first MIndiGolog implementation uses to coordinate online
execution of a shared program. This algorithm depends crucially on
each agent producing the same {}``next step'' at every iteration,
a restriction that is difficult to maintain when each agent may have
a different perspective on the state of the world.
...
any branching from the plan. But, he must account for the fact that
$Joe$ does not have this information, and will have to allow for
several outcomes in the next step. Likewise, $Joe$ must account for
the different local states he considered $Jim$ may have.
...
to the case of incomplete information, the agents must identify a
joint execution that is \emph{commonly known} to be a valid next step.
Unfortunately the situation calculus does not offer any tools for
performing such reasoning. In Chapter TODO we explore the beginnings
of such a formalism, but have not yet closed to loop and used it to
implement coordination of online planning.

TODO: clean this waffle up.
...
\Dt\,\models\,\mathcal{P}(do(\{a\},do(\{a'\},s)))\equiv\mathcal{P}(do(\{a'\},do(\{a\},s)))\\
\Dt\,\models\,\mathcal{F}(do(\{a\},do(\{a'\},s))=\mathcal{F}(do(\{a'\},do(\{a\},s)))\\
\Dt\,\models\, mIndep(c)\,\rightarrow\,\left(Poss(c,s)\equiv\,\forall a\in c:\, Poss(\{a\},s)\right)\\
\Dt\,\models\, mIndep(c)\,\rightarrow\,\left(o\in Out(c,s)[agt]\equiv\exists a\in c:\, o\in Out(\{a\},s)[agt]\right)\\
\Dt\,\models\, mIndep(c)\,\rightarrow\,\mathcal{F}(do(c,s))=\mathcal{F}(do(\{a'\},do(\{a\},s)))\end{gather*}
...
rather than a raw situation term. To make things more concrete, Figure
\ref{fig:plan-output} shows the output of our system when run on
the $MakeSalad$ that was used in that chapter (Figure \ref{fig:MIndiGolog:MakeSalad2}).
...
\caption{Joint Execution for MakeSalad}
...
the agents, with each processing a different ingredient and only synchronising
...
To demonstrate the ability of joint executions to handle branching,
consider the program {}``MakeSalad2'' shown in Figure \ref{fig:MIndiGolog:MakeSalad2}.
Here the agents are unsure whether there are any eggs available. If
there are eggs then they should make an egg salad, otherwise they
should make the standard vegetable salad. Note that since lettuce
appears in both dishes, they are permitted to begin processing theat
ingredient before checking for the eggs.
...
\mathbf{if}\,\,\exists e:\, IsType(e,Egg)\wedge\neg Used(e)\\
\mathbf{then}\\
...
is shown in Figure \ref{fig:JE:MakeSalad2-Exec}. Here we see that
$Joe$ can indeed proceed with preparing the lettuce without knowing
whether eggs are available. $Jim$ is assigned to check for the eggs,
and acquires either an egg or a tomato depending on the outcome of
his sensing action. Importantly, $Jon$ has to wait until $Jim$ acquires
his ingredient before he knows whether to process the cheese, or the
carrot. This highlights the knowing-what restriction discussed in
Section \ref{sec:JointExec:BG:Feasibility}.
...
The $Trans$ and $Final$ predicates of MIndiGolog are modified to
generate \emph{steps} instead of constructing a new situation term.
...
\end{itemize}
These steps contain enough information for the planner to determine
whether two actions can be performed independently. A sequence of
steps corresponds to a history in the obvious way.
