...
for such use. By limiting ourselves to \emph{reasonable} joint
...
the agents are guaranteed to be able to perform it in a purely reactive
fashion, with each agent acting based only on its local information.
...
what actions each agent can observe, rather than requiring the public
...
planning, since the correctness of the algorithm depends crucially on all
agents having the same knowledge of the domain.
