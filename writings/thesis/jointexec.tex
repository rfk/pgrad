...
We will say that a branch can \emph{generate a view} if one of its
histories corresponds to the given view:\[
...
identify a branch, and therefore cannot be used to determine whether
an event is enabled. Agents can only be expected to act based on their
local information, so we must ensure that this information is sufficient
for each agent to determine what actions to perform. This \emph{feasibility}
is ensured using two structural restrictions.
...
between the two events. This is formalised by the following requirement:\begin{multline*}
KnowsWhen(ex)\,\isdef\,\\
...
steps can be used as a history term in the obvious way, taking only
...
as input, returning an new step of execution along with the remainder
...
as normal using the input history. The planner ensures that this is
the canonical history of the leaf that is being planned for, so we
can be sure that the test will hold in all possible histories of the
leaf if it holds in the given history.
...
concurrently while forcing independent actions to occur in a particular
order.
...
The joint execution enumerates all possible outcomes of the action
...
new leaves so generated is also guaranteed to be legal. The new joint
execution is returned along with the new outcome events.
...
branches that have identical views for the performing agent. Currently
this is done using a brute-force search through all possible histories,
but it is able to eliminate many branches quickly by checking their
enablers for incompatible observations.
...
at a time. By ensuring that the joint execution is \emph{feasible},
