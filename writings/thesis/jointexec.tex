...
output of the standard ConGolog execution planning process, is the
\emph{situation}: a complete, ordered sequence of all actions that
are to be performed. This is suboptimal for representing plans in
an asyncrhonous multi-agent setting in three distinct ways:
...
and branch on the sensing results of actions, but restricted enough
...
identify several restrictions on a joint execution that ensure it
is feasible to execute in the real world.
...
and the branching InidGolog plans of \citep{giacomo04sem_delib_indigolog}.
...
While \citet{son00htn_golog} add partial-order operators to the Golog
language, the actual plans produced by their system are still raw
situation terms. One exception is \citep{plaisted97sc_aspect}, which
extends the situation calculus with explicit {}``aspects'' and allows
partial-ordering between actions that affect different aspects of
the world state.

Partial-order planning is the mainstay of the closely-related formalism
of the event calculus. Here action occurrences are specified by asserting
that they happen at a particular time, and the constraints on relative
occurrence times of actions determine a partial ordering. \citet{Shanahan97ec_planning}
...
mirror various concepts from the literature on explicit partial-order
planning, such as threats, links and conflicts \citep{peot92conditional_nonlinear}.
...
to achieve the goal, the agent does not know which one to take. Typically,
feasibility is ensured by including sensing actions to ensure that
the test conditions are known:\[
sense_{\phi}\,;\,\mathbf{if}\,\phi\,\mathbf{then}\, action_{1}\,\mathbf{else}\, action_{2}\]


This informal requirement that an agent {}``knows how'' to execute
a plan is formalised by various notions of \emph{epistemic feasibility},
...
is to allow a plan to be a program formulated in a control language
such as Golog. One then semantically characterise those programs that
are epistemically feasible, using direct assertions about the knowledge
of each agent at each stage of execution. While this allows for potentially
very rich, very succinct succinct plans, it is not clear how to systematically
generate an epistmically feasible plan from such a general characterisation.
...
this is all that is necessary to ensure the feasibility of executing
a plan.
...
that each agent will know when to perform its action.
...
a potential partial run of execution of the system. For convenience,
we define our prime event structures in a slightly different form:\[
...
As shown in \citep{pratt91modeling_conc_with_geom}, these structures
are equivalent to a variety of formalisms for representing concurrent
execution (including schedules and petri nets) and it is straightforward
