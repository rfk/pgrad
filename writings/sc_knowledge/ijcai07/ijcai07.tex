...
model of communication as sensing actions. We also show how to reason
effectively in this formalism by extending the regression operator
...
this is not assumed (e.g. \cite{Lesperance99sitcalc_approach}) the
opposing extreme is posited - that agents are completely ignorant
of any actions performed by others.

Both of these assumptions are unrealistic in many domains. Instead,
...
that spans these two extremes.

It is frequently infeasible for agents to determine what actions have
occurred. Instead, they may simply be aware that the value of a fluents
has changed. By including a special class of actions that notify agents
about fluent change, we show that such domains can be easily accommodated
by our formalism.

Multi-agent systems typically require communication between agents,
usually represented by explicit communication actions. Such actions
are intricately connected with agents' knowledge, and existing accounts
directly include them in the dynamics of knowledge change. It should
not be necessary to alter such fundamental axioms to add or modify
communication actions, and we propose a representation of communication
actions that allows them to be introduced with ease.

Our new semantics of knowledge utilizes quantification over situation
terms, traditionally a roadblock for automated reasoning. Despite
this, we show that the regression operator of \cite{reiter91frameprob}
can be extended to work with our formulation. The final result is
a powerful new account of multi-agent knowledge that still permits
a suitably efficient reasoning procedure.
...
to become false''. This monotonic solution to the frame problem is
a principle attraction of the situation calculus.
...
in uniform formulae to simplify the presentation. It is also standard
practice to pass uniform formulae as arguments to other functions
or predicates - this technically requires an encoding of formulae
as terms, as discussed in \cite{shapiro01casl_feat_inter}. We omit
such details for simplicity.
...
While a very powerful formalism, this formulation suffers from several
limitations:
...
of knowledge 
\end{itemize}
No commitment is made as to how this {}``awareness'' of action occurrances
is generated, with responsibility abdicated to a separate component
of the agent's software. By adopting instead the form of successor
state axiom proposed in \cite{Lesperance99sitcalc_approach}, we get
the opposite extreme: agents are only aware of actions that they themselves
perform: \begin{multline}
K(agt,s'',do(c,s))\equiv\\
...
compatible with the actions that they have performed. There may have
...
of exogenous (from $agt$'s point of view) actions. Again, there are
...
principle tool for such reasoning is the regression meta-operator
\cite{reiter91frameprob}, which transforms a formula $\phi$ uniform
in $do(c,s)$ into a formula $\mathcal{R}(\phi)$ uniform in $s$,
such that the two are equivalent relative to the theory of action
$\Sigma$. By repeatedly applying the regression operator, $\phi$
can be transformed into an equivalent formula uniform in the initial
situation:

\[
\Sigma\models\phi[do(c_{1},do(c_{2},\dots,do(c_{n},S_{0}))]\equiv\mathcal{R}^{*}(\phi)[S_{0}]\]
...
$\mathcal{R}^{*}(\phi)[S_{0}]$. This is generally more efficient
than directly reasoning about $\phi$ using all the axioms in $\Sigma$.
...
offer no procedure for reasoning in their formalism other than the
general approach of using second-order induction described in \cite{Reiter93proving}.

We show that the regression idea can be extended to limited forms
of quantification over situations, providing a purely first-order
...
$agt$ would be able to observe action $a$ being performed in situation
...
us also introduce the predicate $Legal(s)$, which states that a situation
can legally be brought about in the world. For this basic situation
calculus, it suffices that it constrains actions to be possible: \begin{multline}
Legal(S_{0})\equiv True\\
\shoveleft{Legal(do(c,s))\equiv Poss(c,s)\wedge Legal(s)}\end{multline}


The following successor state axiom for knowledge capture the desired
semantics: \begin{multline}
K(agt,s'',do(c,s))\equiv\\
...
captures the intended semantics: \begin{equation}
K(agt,s,S_{0})\equiv Legal(s)\wedge\exists s'\,\left[K_{0}(agt,s')\wedge\mathbf{Unobs}(agt,s',s)\right]\label{eqn:new_k_s0}\end{equation}
...
is not aware of any actions occuring. It is thus a true generalization
...
reflexive and euclidian relations. We omit proofs for space reasons.

TODO: is it? then better sit down with pen and paper and do so!


\subsubsection{Awareness of Fluent Change}

In many domains, it may not be feasible for an agent to infer that
...
of precisely which action took place.
...
the resulting situation. By making agents aware of such actions using
...
unpublished work (TODO:cite ourselves) we introduce a new meta-operator
$\mathcal{P}(\phi,\alpha)$, taking a uniform formula $\phi$ and
some conditions $\alpha(c,s)$ on what actions may occur, such that:
\begin{multline}
\Sigma\models\mathcal{P}(\phi,\alpha)[s]\equiv\forall s''\, s\leq s''\wedge\\
...
that, as long as all actions satisfy $\alpha$, the formula $\phi$
will remain true in all situations in the future of $s$. It thus
represents the conditions under which $\phi$ will \emph{persist}
with respect to a given class of actions. The procedure for determining
$\mathcal{P}$ replaces second-order induction with iterated first-order
reasoning, and always gives a uniform formula.
...
\left.\mathcal{R}(\mathcal{P}(\phi,Legal()\wedge Observations(agt)=\{\})[do(c',s')]),s)\right)\end{multline}
...
\mathcal{R}(\mathbf{Knows}(agt,\phi,S_{0}))=\\
\forall s\, K_{0}(agt,s)\rightarrow\mathcal{P}(\phi,Legal()\wedge Observations(agt)=\{\})[s]\end{multline}


This is then standard modal reasoning over the $K_{0}$ relation.
It remains of course to prove that the regression operator works as
required, and we sketch the highlights of such a proof below:

\begin{theorem}

Given a theory of action $\Sigma$ with regression and peristence
operators defined relative to it, and a ground situation term $s$,
it is always the case that:

\[
\Sigma\models\phi[s]\equiv\mathcal{R}^{*}(\phi)[S_{0}]\]
...
It suffices to show that $\mathcal{R}$ preserves equivalence for
$\mathbf{Knows}$, as that it all we have modified. Employing the
successor state axiom from equation \ref{eqn:new_k_ssa}, the definition
of the $\mathbf{Knows}$operator from equation \ref{eqn:knows_def}
...
\end{proof}
 ...
 We can thus reduce reasoning about knowledge in a given situation
to modal reasoning in the initial situation, as before. Our extension
of the regression operator thus provides a mechanical procedure for
reasoning in our improved account of knowledge without requiring second-order
induction. 
