...
\title{Property Persistence in the Situation Calculus}
...
can be thought of as formulae that describe a \emph{property} of the
...
\[
\forall s\,.\, s\sqsubset do(a,s')\equiv s\sqsubseteq s'\]
...
\[
\forall s\,.\, s<do(a,s')\equiv s\leq s'\wedge Poss(a,s')\]
...
\[
\forall s\,.\, s<_{\alpha}do(a,s')\equiv s\leq_{\alpha}s'\wedge\alpha(a,s')\]
...
\subsection{Property Persistence}
...
query problems outlined previously. Since $\mathcal{P}_{\mathcal{D}}$
yields a uniform formula, the techniques outlined in Section \ref{sub:Effective-Reasoning}
