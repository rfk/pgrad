...
\begin{frame}
\frametitle{Representing Executions}
...
\item A labelling function $\gamma(e) = l$
\end{itemize}
\ \\
\ \\
...
\begin{frame}
\frametitle{Independent Actions}
Assume we can identify independent actions, $indep(a_1,a_2)$, such that:
\begin{itemize}
\item $\Dt \models Poss(a_1,s) \equiv Poss(a_1,do(a_2,s))$
\item $\Dt \models Out(a_1,s) \equiv Out(a_1,do(a_2,s))$
\item $\Dt \models \mathbf{Q}(do(a_1,do(a_2,s))) \equiv \mathbf{Q}(do(a_2,do(a_1,s)))$
\item $\Dt \models \mathbf{F}(do(a_1,do(a_2,s))) = \mathbf{F}(do(a_2,do(a_1,s)))$
\end{itemize}
\ \\
\ \\
Situations that are identical up to re-ordering of independent actions are equivalent.
\end{frame}

\begin{frame}
\frametitle{Joint Executions}
We define \emph{joint executions} as a special kind of prime event structure:
\begin{itemize}
\item Disjoint sets of \emph{action} events $\mathcal{A}$ and \emph{outcome} events $\mathcal{O}$
\item A total order $<$ that is consistent with $\prec$
\end{itemize}
\ \\
\ \\
Given a set of events $e$ from the JE, we can use $<$ to reduce it to a unique
situation term $hist(e)$.
\end{frame}

\begin{frame}
\frametitle{Joint Executions}
\begin{center}
  picture of example JE goes here
  %\includegraphics[scale=0.35]{pk_regression}
\end{center}
\end{frame}

\begin{frame}
\frametitle{Restrictions on Joint Executions}
To ensure a joint execution can actually be executed, we enforce the following 
restrictions:
\ \\
\ \\
\begin{itemize}
...
\ \\
$\ \ \forall i_1,i_2 \in \mathcal{A} :\ \  i_1 \prec i_2\ \vee\ i_2 \prec i_1\ \vee\ indep(\gamma(i_1),\gamma(i_2))$\\
\ \\
\item Every canonical history is legal: \\
\ \\
$\ \ \forall e \subset \mathcal{O} :\ \ Legal(hist(e))$ 
\end{itemize}
\end{frame}
...
\ \\
$\ \ \Dt \not\models Out(\gamma(i)) \neq r\ \ \Rightarrow\ \exists j : ens(j) = \{i\} \wedge \gamma(j)=r$\\
\ \\
\item Actions are enabled by observable events:\\
\ \\
$\ \ \forall i \in \mathcal{A} : \forall j \in ens(i) :\ \ \gamma(j)[actor(\gamma(i))] \neq \{\}$\\
\ \\
\item Overlapping views enable identical actions:\\
\ \\
$\ \ \forall i \in \mathcal{A} : \forall e \subset \mathcal{O} :$\\
$\ \ \ \ \ overlaps(e,ens(i)) \rightarrow \exists j: \gamma(j)=\gamma(i) \wedge ens(j)=e$
\end{itemize}
\end{frame}
